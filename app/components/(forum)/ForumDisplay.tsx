...
const ForumDisplay = ({ forum }: Props) => {
  ...
  return (
    <div className="w-[80%] flex gap-2 flex-col items-end">
      ...
      <Link href={`../Thread/${thread?.id}`} className={`bg-green dark:bg-gradient-to-r from-orange to-pink rounded-full dark:rounded-none text-soft-pink dark:text-dark-grey text-xs w-[60%] ${typeof thread === 'undefined' ? 'hidden': ''} cursor-pointer hover:drop-shadow-xl  relative hover:bottom-[2px] hover:right-[3px]`}>
        <div className="p-2">
